...
# rucnyz/harbor fork) + Qwen3.5-9B + 256K context, FULLY ASYNC trainer
# (in-flight weight update / multi-turn partial rollout).
#
# Smoke-scale defaults: 4 GPUs (2 FSDP train + 2 vLLM engines), 8-prompt
# mini-batch x 4 samples against the 16-task smoke dataset. Scale knobs up
# once the pipeline is verified.
#
# Requires: kernel.apparmor_restrict_unprivileged_userns=0 (Ubuntu 24.04+)
# — see /etc/sysctl.d/99-nitrobox-userns.conf.
...
# Directory setup
#-----------------------
RUN_NAME="${RUN_NAME:-nitrobox_async_smoke_v1}"
...
#----------------
# Infrastructure setup. 4 GPUs: 2 vLLM engines + 2 FSDP train.
#----------------
...
uv run --isolated --extra fsdp --extra harbor -m examples.train_integrations.harbor_pgc.entrypoints.main_harbor_fully_async \
